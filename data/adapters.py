...
class TessAdapter(BaseTelescopeAdapter):
    ...
    def get_query(self) -> str:
        return """
        SELECT toi, toidisplay, toipfx, tfopwg_disp, pl_orbper, pl_rade, pl_insol,
               pl_eqt, st_teff, st_rad
        FROM toi
        WHERE tfopwg_disp = 'PC' OR tfopwg_disp = 'KP' OR tfopwg_disp = 'CP'
        """

    def standardize(self, df: pd.DataFrame) -> pd.DataFrame:
        period = pd.to_numeric(df["pl_orbper"], errors="coerce")
        sma = (period / 365.25) ** (2.0 / 3.0)
        return pd.DataFrame({
            "name": df["toidisplay"].astype(str),
            "host_name": "TOI-" + df["toipfx"].astype(str),
            "source": self.mission_name,
            "disposition": df["tfopwg_disp"],
            "period": period,
            "radius": pd.to_numeric(df["pl_rade"], errors="coerce"),
            "mass": pd.NA,
            "insol": pd.to_numeric(df["pl_insol"], errors="coerce"),
            "eq_temp": pd.to_numeric(df["pl_eqt"], errors="coerce"),
            "semi_major_axis": sma,
            "eccentricity": 0.0,
            "inclination": 90.0,
            "st_teff": pd.to_numeric(df["st_teff"], errors="coerce"),
            "st_radius": pd.to_numeric(df["st_rad"], errors="coerce"),
            "st_mass": 1.0,
        })
